 ...
 */
export function createAgentEngine({ store = null, ManagerClass: injectedManager = null } = {}) {
  ...
  return {
    ...
    async runAgent({ message, accessToken, instanceUrl, sessionKey, onEvent }) {
      ...
      try {
        ...
        const result = await manager.handleMessage(message, accessToken, instanceUrl, onEvent);
