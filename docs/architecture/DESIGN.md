...
| Surface | Personality |
|---|---|
| **Root `/`** | **Live Blueprint landing** (Pass 40 + 42, +client shaping Pass 44): light technical schematic world — dot-grid blueprint texture over a fixed ambient drafting-line layer, Inter display type (DESIGN.md font), a looping `LivePipeline` chat preview (message → route → gates → signed record; in-view gated, hover-pause, reduced-motion static), an isometric SVG drafting plane + scroll-linked parallax behind the hero (Pass 42), blurred wireframe `AmbientShape` silhouettes in every section, scroll-drawn connector rail in How-it-works, and one dark forge CTA anchor. **Client-first arc (Pass 44):** the page leads with the client's problem (`#problem` — a verified before/after split: one formula edit breaking a revenue integration → impact analysis, ten refusal gates, dry-run, signed trail) and adds a PRD §4 persona map (`#audience` — Admin/Architect, Release/Change Manager, DevOps/Platform, Exec/Reviewer, each tied to its real surface). Full spec: `LANDING_REDESIGN_PLAN.md` |
...
| **Dashboard** | Production command center (Pass 45): hero row + 3 hover-lift KPI tiles (mono numerals, color-coded accent hairlines) + one banner + a data-viz row (14-day change-activity SVG bar chart with hover tooltip, change-status SVG donut with text legend) + an operational row (OrgHealthPanel: readiness checks as a segmented progress list; AgentsOverview: live agent snapshot) + the activity feed. All charts are hand-rolled SVG over real fetched data (records, agents, readiness diag) — zero chart dependencies, zero fabricated metrics; empty states keep the axes and point to chat. Empty states still collapse to one CTA |
