...
```
orgforge/
├── backend/                   @orgforge/api   — ONE Express 5 entry (:3001), mounts all routers
├── frontend/                  @orgforge/web   — Next.js 16 App Router app (:3000)
├── packages/
│   ├── ai/                   @orgforge/ai    — routeIntent classifier + SSE envelope + describeImage (Gemini)
│   ├── auth/                 @orgforge/auth  — requireAuth + tenantIsolation (Supabase JWT)
│   ├── diagnostics/          @orgforge/diagnostics — pre-flight port + server-side 24h cache (EC-14)
│   └── org-connections/      @orgforge/org-connections — encrypted credential store, refresh dedup, re-link
...
- **Auth wiring:** `@orgforge/auth` `requireAuth` + `tenantIsolation` middleware; Supabase JWT verified server-side via `supabase.auth.getUser`; dev sessions in localStorage, **production uses httpOnly Secure cookies (D2)**.
...
|---|---|---|
| `@orgforge/ai` | `routeIntent` classifier (Gemini + deterministic §7.1 overrides, stub-able for tests/canary), unified SSE envelope types, `describeImage` (Gemini vision for org-change image attachments) | `@google/genai` |
| `@orgforge/auth` | `requireAuth`, `tenantIsolation` | `@supabase/supabase-js`, jsonwebtoken (dev) |
| `@orgforge/diagnostics` | pre-flight port (license / package-by-SubscriberPackageId / provisioning / org-type), server-side 24h cache + promise dedup, `invalidateDiagnostics` (EC-14), package-missing never-pin (Pass 22) | — |
| `@orgforge/org-connections` | encrypted credential store (`iv:authTag:encryptedData`, AES-256-GCM), per-org refresh dedup, legacy re-link (RPC-backed) | jsonwebtoken |
...
- **One classifier, one envelope** (`@orgforge/ai`): `routeIntent` decides `agent | org_change | both | clarify` with Gemini + deterministic overrides; capability chip pins bypass the classifier; every decision → `orgforge.routing_log`.
...
- **Backend:** `node:test` across `@orgforge/api` + all packages — **402/402** passing (unit: routes, engines, stores, classifier, SSE, crypto, verifySchema; re-homed OrgForge suite 138; S-2 degrade paths).
...
- **Dev:** `npm run dev:api` + `npm run dev:web`; `.env.example` at root + `frontend/.env.example` — **canonical env names** (SALESFORCE_*, GOOGLE_AI_API_KEY, SUPABASE_SERVICE_ROLE_KEY, ORGFORGE_* flags) with legacy Agentforge/OrgForge aliases honored as fallbacks; `JWT_SECRET` retired (Pass 36).
- **Prod:** Next.js on Vercel + Express API on Render; Supabase hosted Postgres; Redis (managed).
- **Feature flags:** `ORGFORGE_UNIFIED_API=on` (capability routers), `ORGFORGE_UNIFIED_FRONTEND` (canary chip), `ORGFORGE_ECA_PACKAGE_VERSION_ID` (package check override).
